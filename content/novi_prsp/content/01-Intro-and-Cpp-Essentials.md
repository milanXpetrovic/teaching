...
#### Rad s datotekama
Za natjecanja gdje je ulaz/izlaz u datotekama (npr. `input.txt`, `output.txt`), možete preusmjeriti standardne streamove:
```cpp
...
Ova skripta pruža studentima sve što im je potrebno za početak: motivaciju, praktične alate, osnovne strukture i smjernice za vježbu. Sadržaj je namjerno sažet i direktan, u duhu natjecateljskog programiranja.
